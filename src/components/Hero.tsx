                ...
                <div className="lg:grid lg:grid-cols-2 lg:gap-16 items-center">
                    ...
                    <motion.div initial={{ opacity: 0, x: -50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
                        <h1 className="text-4xl lg:text-6xl font-black text-db-dark leading-[1.1] mb-6">
                            Reach <span className="text-db-teal">10+ Crore</span> Readers
                        </h1>
                        ...
                        <ul className="space-y-3 mb-10">
                            {["Print + Digital Reach", "65+ Editions", "300+ District Coverage"].map((item, index) => (
                                <li key={index} className="flex items-center text-base font-bold text-db-dark group">
                                    {/* Checklist icons: teal default, yellow on group hover */}
                                    <div className="w-5 h-5 bg-teal-50 rounded-full flex items-center justify-center mr-3 group-hover:bg-amber-50 transition-colors duration-300">
                                        <CheckCircle2 className="w-3.5 h-3.5 text-db-teal group-hover:text-amber-500 transition-colors duration-300" />
                                    </div>
                                    {item}
                                </li>
                            ))}
                        </ul>
                        {/* Primary CTA: teal (trust & stability) */}
                            ...
                            {isSuccess ? (
                                ...
                                <>
                                    <div className="flex justify-between items-start mb-8">
                                        ...
                                        {/* Step indicators: completed = teal, active = yellow accent */}
                                        <div className="flex gap-2">
                                            ...
                                            <div className={`h-2 w-8 rounded-full transition-all duration-500 ${step >= 2 ? 'bg-amber-400 shadow-md shadow-yellow-100' : 'bg-slate-100'}`} />
                                        </div>
                                    </div>
                                    ...
                                    <AnimatePresence mode="wait">
                                        {step === 1 ? (
                                            <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4 flex-1">
                                                ...
                                                {/* Next Step: teal primary (trust/progression) */}
                                            ...
                                            <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex-1 flex flex-col">
                                                ...
                                                <div className="mb-4">
                                                    ...
                                                    {showCalendar && <div className="bg-slate-50 rounded-2xl border border-slate-100 p-3">
                                                                ...
                                                                return (
                                                                    <button
                                                                        ...
                                                                        className={[
                                                                            ...
                                                                            // Selected date: yellow accent for action/energy
                                                                            selected ? 'bg-amber-400 text-gray-900 shadow-md font-black' : '',
                                                                            // Today: teal border (primary brand)
                                                                            !selected && todayDay ? 'border border-db-teal text-db-teal font-black' : '',
                                                                            !selected && !past && !todayDay ? 'text-slate-700 hover:bg-amber-50 hover:text-amber-600' : '',
                                                                        ].join(' ')}
                                                                    >
                                                                        {day}
                                                                    </button>
                                                        ...
                                                        {/* Selected date label */}
                                                        {formData.date && (
                                                            <div className="mt-2 pt-2 border-t border-slate-200 text-center text-[10px] font-black text-amber-600 uppercase tracking-wider">
                                                ...
                                                <div className="mb-2">
                                                    <button
                                                        ...
                                                        className={`w-full flex items-center justify-between gap-3 mb-2 px-3 py-2 rounded-xl transition-colors ${!formData.date ? 'text-slate-300 cursor-not-allowed' :
                                                            showTimeSlots ? 'bg-amber-50 text-amber-600' : 'text-slate-400 hover:bg-slate-50'
                                                            }`}
                                                    >
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            {timeSlots.map(slot => (
                                                                <button
                                                                    ...
                                                                    className={`py-2.5 px-1.5 rounded-xl text-[10px] font-black border transition-all duration-200 ${formData.timeSlot === slot
                                                                        // Selected slot: yellow accent
                                                                        ? 'bg-amber-400 text-gray-900 border-transparent shadow-md scale-[1.03] z-10'
                                                                        : 'bg-slate-50 border-slate-100 text-slate-600 hover:border-amber-300 hover:bg-amber-50 hover:text-amber-600'
                                                                        }`}
                                                                >
                                                                    {slot}
                                                                </button>
                                                ...
                                                <div className="flex gap-3 mt-auto pt-3">
                                                    ...
                                                    {/* Confirm Booking: teal primary (trust/action to confirm) */}
                                                    <button
